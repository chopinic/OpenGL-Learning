...
int LoadGLTextures()                                    // Load Bitmaps And Convert To Textures
{
	...
	if ((TextureImage[0] = LoadBMP("Font.bmp")) &&
		(TextureImage[1] = LoadBMP("Bumps.bmp")))
	{
...
GLvoid BuildFont(GLvoid)								// Build Our Font Display List
{
	...
	for (loop = 0; loop < 256; loop++)						// Loop Through All 256 Lists
	{
		...
		glVertex2i(fsize, 0);						// Vertex Coord (Bottom Right)
		glTexCoord2f(cx + 0.0625f, 1 - cy);			// Texture Coord (Top Right)
		glVertex2i(fsize, fsize);						// Vertex Coord (Top Right)
		glTexCoord2f(cx, 1 - cy);					// Texture Coord (Top Left)
		glVertex2i(0, fsize);						// Vertex Coord (Top Left)
		glEnd();									// Done Building Our Quad (Character)
		glTranslated(fsize/8*5, 0, 0);						// Move To The Right Of The Character
